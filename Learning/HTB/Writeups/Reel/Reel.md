...
- *With BloudHound,* we find that nico has a `WriteOwner` right over another user `herman`.
- We abuse this right to grant ourselves the right to reset his password. We then find out that `herman` has a `WriteDACL` over a certain group called `Backup_Admins`.
- We add ourselves to that group and find that it has access to a certain folder called `Backup Scripts` on the `Administrator`'s desktop on the box.
...
- A *Similar path* exists with another user `tom` whom his credentials are present in a `creds.xml` file as a `secure string` on `nico`'s desktop.
