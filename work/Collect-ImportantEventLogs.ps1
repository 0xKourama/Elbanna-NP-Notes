...
elseif($ShowEventInformation){
    ...
    $Event_Data = @(
		...
		[Event]@{
            ...
            Description='Failed user logins can show possible password spray and password guessing attacks. Not every failed login is an attack, but they can be early indicators of one. This log is recquired by the PCI DSS regulation and is recommended by the NSA Cyber Event Forwarding Guidance.'
 		}
		[Event]@{
			Logname='Security'    
            Provider='N/A'         
		...
		[Event]@{
            ...
            Description='Groups allow for multiple user accounts to be managed as one and an attacker may try and create a group with escalated privileges.'
 		}
		[Event]@{
			Logname='Security'    
            Provider='N/A'         
            ID=4735 
            Title='Group Change'
            Description='Group change can indicate unauthorized privilege escalation of a whole user group. This should be monitored so that user permissions are known and not over reaching. This log is recommended by the NSA Cyber Event Forwarding Guidance.'
 		}
		[Event]@{
			Logname='Security'    
            Provider='N/A'         
		...
		[Event]@{
            ...
            Description='Group membership change can indicate a user removing themselves from an admin group in an effort to clean up after an attack. It is important to monitor the groups that have escalated permissions.'
 		}
		[Event]@{
			Logname='Security'    
            Provider='N/A'         
            ...
            Description='Group deletion may signal the cleanup of an attack or someone trying to inconvenience a group of users in a network. It is also good to know what groups are active.'
 		}
        [Event]@{
        	Logname='System'      
            Provider='N/A'         
            ...
            Description='Services on a Windows machine have many purposes and these logs may indicate a malicious service being places on the computer. This log is recommended by JPCERT.'
 		}
        [Event]@{
        	Logname='Application' 
            Provider='N/A'         
            ...
            Description='Many attack rely on software being installed and this log can signal who and what installed the software that may cause a vulnerability. It can also be used to make sure that only authorized apps are being installed.'
 		}
        [Event]@{
			Logname='Application' 
            Provider='N/A'         
            ...
            Description='Software removal may indicate previously used malicious software being taken off a machine or an attempt to disrupt the normal workflow.'
        }
        [Event]@{
    		Logname='Application' 
		    ...
		    Description='Software updates are not always a sign of an attack, but often contain patches for known vulnerabilities. An update may also introduce vulnerabilities and it is important to know if and when that was installed.'
	    }
    )
