...
function _GetDBTable
{
	...
	foreach ($Database in $Databases)
	{
        ...
        if (($Database.Size/1GB) -gt 2500)
        {
		    $Output+="<td align=""center"" bgcolor=""#ff0000"">$("{0:N2}" -f ($Database.Size/1GB)) GB </td>
		...
		if ($ShowFreeDatabaseSpace)
		{
			if ($Database.FreeDatabaseDiskSpace -lt 25)
			{
				$Output+="<td align=""center"" bgcolor=""#ff0000"">$("{0:N1}" -f $Database.FreeDatabaseDiskSpace)%</td>"
			}
			    else
			    {
		...
		if ($ShowFreeLogDiskSpace)
		{
			if ($Database.FreeLogDiskSpace -lt 25)
			{
				$Output+="<td align=""center"" bgcolor=""#ff0000"">$("{0:N1}" -f $Database.FreeLogDiskSpace)%</td>"
			}
			    else
			    {
		...
		if ($ShowCopies)
		{
			if ($Database.CopyCount -lt 3)
			{
				$Output+="<td bgcolor=""#ff0000"">$($Database.Copies|%{$_}) ($($Database.CopyCount))</td>"
			}
			    else
			    {
...
Send-MailMessage -To mgabr@roaya.co -From ExOrg@roaya.co -Subject "Exchange Environment Report" -BodyAsHtml $Output -SmtpServer 192.168.3.202