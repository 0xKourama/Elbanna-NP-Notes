...
$Csv = Import-Csv 'Domains.csv' | Sort-Object -Property Domain
        ...
        if($MX_Query.NameExchange){
            foreach($MX_Result in $MX_Query){
                switch -Regex ($MX_Result.NameExchange){
                    'mx1.domain.com'{$Obj.MX1 = $true}
                    'mx2.domain.com'{$Obj.MX2 = $true}
                    'mx3.domain.com'{$Obj.MX3 = $true}
                    'mx4.domain.com'{$Obj.MX4 = $true}
        ...
        if($TXT_Query){
            foreach($TXT_Result in $TXT_Query){
                Switch -Regex (($TXT_Result.strings | Out-String).trim()){
                    'v=spf1 mx include:_spf.domain.com -all'{$Obj.SPFConfigured = $true}
                    'WorldPosta-verification=.*'{$Obj.DKIMConfigured = $true}
                    'v=DMARC1; p=none; rua=mailto:report@domain.com; ruf=mailto:report@domain.com; sp=none;'{$Obj.DMARCConfigured = $true}
                }
            }
        }        
    }
    ...
    try{
        ...
        if($SRV_Query){
            foreach($SRV_Result in $SRV_Query){
                if($SRV_Result.NameTarget -eq 'autodiscover.domain.com'){
